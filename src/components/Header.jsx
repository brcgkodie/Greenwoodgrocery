...
export default function Header() {
  ...
  const navClass = (to) =>
    `hover:text-forest transition-colors ${pathname === to ? "text-forest font-semibold" : ""}`;
  ...
  return (
    <header className="sticky top-0 z-40 backdrop-blur-md bg-cream/85 border-b border-forest/10">
      <div className="max-w-6xl mx-auto px-5 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="md:hidden text-forest/70 hover:text-forest"
          >
      ...
      {/* Mobile nav */}
      {mobileOpen && (
        <nav className="md:hidden border-t border-forest/10 bg-cream px-5 py-4 space-y-3">
          {links.map((l) => (
            <Link
              key={l.to}
              to={l.to}
              onClick={() => setMobileOpen(false)}
              className={`block text-sm text-forest/70 ${navClass(l.to)}`}
            >
              {l.label}
            </Link>
          ))}
